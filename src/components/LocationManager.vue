...
const playNotificationSound = () => {
  try {
    ...
    // Для Android требуется пользовательское взаимодействие
    const playPromise = audio.value.play();
    
    // Обработка ошибки автовоспроизведения на некоторых браузерах
...
const testNotification = () => {
  ...
  if ('Notification' in window && Notification.permission === 'granted') {
    try {
      // Проверка, поддерживаются ли сервис-воркеры
      if ('serviceWorker' in navigator) {
        navigator.serviceWorker.ready.then(registration => {
          // Использование сервис-воркера для отправки уведомления (работает на Android)
          registration.showNotification('Семейное приложение', {
            body: 'Вова возвращается домой!',
            icon: '/icon-192.png',
            vibrate: [200, 100, 200], // Вибрация для Android
            tag: 'homecoming', // Идентификатор для группирования уведомлений
        ...
        // Запасной вариант для браузеров без поддержки сервис-воркеров
        new Notification('Семейное приложение', {
          body: 'Вова возвращается домой!',
          icon: '/icon-192.png'
        });
      ...
      // Запасной вариант - показать внутреннее уведомление
      alert('Вова возвращается домой!');
    }
  } else if (Notification.permission !== 'denied') {
    // Запрашиваем разрешение, если оно еще не запрошено
    Notification.requestPermission().then(permission => {
      if (permission === 'granted') {
        testNotification(); // Повторная попытка после получения разрешения
      }
    });
  }
};
